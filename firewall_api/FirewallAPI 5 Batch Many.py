...
class Firewall:
    ...
    def _remove_spaces(self, data):
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                data[key] = [
                    self._remove_spaces(item) if isinstance(
                        item, dict) else item
                    for item in value
                ]
    ...
    def batch_crud_operation(self, entity, operation, data_list):
        """
        Execute a standard operation (create, read, update, delete) on multiple entities of the same type.
        
        This method allows for efficient batch processing of CRUD operations on the same entity type.
        
        :param entity: The type of entity to operate on
        :param operation: The operation to perform ('create', 'read', 'update', 'delete')
        :param data_list: A list of parameters specific to each operation:
                         - For 'create': list of entity_data dictionaries
                         - For 'read': list of filter_value strings or parameter dictionaries
                         - For 'update': list of (entity_data, entity_name) tuples or entity_data dictionaries
                         - For 'delete': list of filter_value strings or parameter dictionaries
        :return: A list of dictionaries with the results of each operation
        
        Example usage:
        ```python
        # Create multiple services
        firewall.batch_crud_operation(
            entity='Services',
            operation='create',
            data_list=[
                {'Name': 'Service1', 'Protocol': 'TCP', 'DestinationPort': '80'},
                {'Name': 'Service2', 'Protocol': 'TCP', 'DestinationPort': '443'}
            ]
        )
        
        # Read multiple services
        firewall.batch_crud_operation(
            entity='Services',
            operation='read',
            data_list=['Service1', 'Service2']
        )
        ...
        firewall.batch_crud_operation(
            ...
            operation='update',
            data_list=[
                {'Name': 'Service1', 'DestinationPort': '8080'},
                {'Name': 'Service2', 'DestinationPort': '8443'}
            ]
        )
        ...
        if operation not in ('create', 'read', 'update', 'delete'):
            return [{"status": "400", "message": f"Invalid operation: {operation}", "data": []}]
        ...
        for item in data_list:
            try:
                # Handle each operation type directly
                if operation == 'create':
                    # For create, each item is an entity_data dictionary
                    result = self.create(entity=entity, entity_data=item)
                    
                elif operation == 'read':
                    # For read, each item must be a dictionary
                    result = self.read(entity=entity, **item)
                    
                elif operation == 'update':
                    # For update, each item can be a tuple (entity_data, entity_name) or just entity_data
                    if isinstance(item, tuple) and len(item) == 2:
                        result = self.update(entity=entity, entity_data=item[0], entity_name=item[1])
                    else:
                        result = self.update(entity=entity, entity_data=item)
                        
                elif operation == 'delete':
                    # For delete, each item can be a filter_value string or a parameter dictionary
                    if isinstance(item, dict):
                        # If it's a dictionary, use it as parameters
                        result = self.delete(entity=entity, **item)
                    else:
                        # If it's a string, use it as filter_value
                        result = self.delete(entity=entity, filter_value=item)
